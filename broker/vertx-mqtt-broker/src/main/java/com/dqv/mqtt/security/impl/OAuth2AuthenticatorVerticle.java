...
public class OAuth2AuthenticatorVerticle extends AuthenticatorVerticle {
    ...
    public void startAuthenticator(String address, AuthenticatorConfig c) throws Exception {
    	logger.info("VUDQ startAuthenticator:" + c.getIdpUsername());
        ...
        MessageConsumer<JsonObject> consumer = vertx.eventBus().consumer(address, (Message<JsonObject> msg) -> {
        	...
        	logger.info("VUDQ usernameOrAccessToken:" + usernameOrAccessToken);
            // token validation
            ...
            try {
                if(usernameOrAccessToken!=null) {
                    if (usernameOrAccessToken.contains("@")) {
                        ...
                        Boolean tokenIsValid = oauth2Validator.tokenIsValid(usernameOrAccessToken,passwordOrRefreshToken);
                        TokenInfo info = oauth2Validator.getTokenInfo(usernameOrAccessToken,passwordOrRefreshToken);
                    	logger.info("VUDQ info:" + info);
                    	logger.info("VUDQ tokenIsValid:" + tokenIsValid);
