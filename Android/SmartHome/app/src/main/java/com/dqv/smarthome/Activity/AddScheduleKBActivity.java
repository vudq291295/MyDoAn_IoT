...
public class AddScheduleKBActivity extends AppCompatActivity {
    public static final String TAG = AddScheduleKBActivity.class.getSimpleName();
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                ...
                case R.id.btnAddCheKB:
                    ...
                    model.setStatus(1);
                    if(model.getScriptID() <=0){
                        Toast.makeText(getApplicationContext(),"Hãy chọn kịch bản",Toast.LENGTH_LONG).show();
                    }
                    else{
                        InsertEquipApi(model);
                    }
                    break;
                case R.id.btnCancelCheKB:
                    onBackPressed();
                    break;
                default:
                    break;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_add_schedule_kb);
        Intent intent = getIntent();
        Bundle b = intent.getExtras();
        toolbar = (Toolbar) findViewById(R.id.toolbarAddScheduleKB);
        ...
        timePicker=(TimePicker)findViewById(R.id.alarmTimePickerKB);
        timePicker.setIs24HourView(true);
        currentToken = MyApplication.getInstance().getPrefManager().getUser().getToken();
        mDayChe2 = (TextView)findViewById(R.id.twAddSche2KB);
        mDayChe3 = (TextView)findViewById(R.id.twAddSche3KB);
        mDayChe4 = (TextView)findViewById(R.id.twAddSche4KB);
        mDayChe5 = (TextView)findViewById(R.id.twAddSche5KB);
        mDayChe6 = (TextView)findViewById(R.id.twAddSche6KB);
        mDayChe7 = (TextView)findViewById(R.id.twAddSche7KB);
        mDayCheC = (TextView)findViewById(R.id.twAddScheCKB);
        btnAddChe = (Button) findViewById(R.id.btnAddCheKB);
        btnCancelSche = (Button) findViewById(R.id.btnCancelCheKB);
        etnameSche = (EditText) findViewById(R.id.nameScheKB);
        mSelectTB = (TextView) findViewById(R.id.twSelectKB);
        ...
        if(b!=null){
            ...
            model.setScriptID(b.getInt("scriptID"));
            ...
            mSelectTB.setText(model.getScriptID()+"");
            for(int i =0;i<lstDay.size();i++){
        ...
        mSelectTB.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(getApplicationContext(),SelectScriptActivity.class);
                startActivityForResult(intent,1);
            }
        });
        Log.d(TAG, "onCreate isAdd: " + isAdd);
        if(isAdd){
            tw.setText("Thêm lịch kịch bản");
        }
        else {
            tw.setText("Sửa lịch kịch bản");
        }
    }
        ...
        switch(requestCode) {
            case (1) : {
                if (resultCode == Activity.RESULT_OK) {
                    ...
                    if(b!=null){
                        model.setScriptID(b.getInt("scriptID"));
                        mSelectTB.setText(b.getString("scriptName"));
                    }
                }
                break;
            }
    ...
    public void InsertEquipApi(final ScheduleModel model){
        ...
        try
        {
            ...
            postparams.put("scriptID", model.getScriptID()+"");
            ...
            public void onResponse(JSONObject response) {
                ...
                try {
                    ...
                    if(!response.getBoolean("error")){
                        Intent intent = new Intent(getApplicationContext(), ScheduleKBActivity.class);
